...
static DEFINE_MUTEX(touch_booster_lock);
#endif
/* added header file */
...
static void get_chip_type(void *device_data);
...
static struct tsp_cmd tsp_cmds[] = {
	...
	{TSP_CMD("get_chip_type", get_chip_type),},
...
#if TOUCH_BOOSTER
	u8							finger_cnt;
	struct workqueue_struct		*tb_work_queue;
	struct cpufreq_limit_handle		*min_handle;
	struct work_struct				tb_on;
	struct work_struct				tb_off;
#endif
};
/* Dummy touchkey code */
...
static void bt532_firmware_check(void)
{
	...
	if(!(gpio_get_value(TSP_ID_CHECK90) || gpio_get_value(TSP_ID_CHECK91)))
	{
		...
		misc_info->tsp_type = 3;
	}
	else
	{
		m_firmware_data = m_firmware_data_ito;
		misc_info->tsp_type = 0;
	}
	gpio_free(TSP_ID_CHECK90);
	gpio_free(TSP_ID_CHECK91);
#else
	return;
#endif
}
...
void charger_enable(int status)
{
	...
	if (reported == true ||
		...
		for (i = 0; i < info->cap_info.multi_fingers; i++) {
			...
			if (zinitix_bit_test(prev_sub_status, SUB_BIT_EXIST)) {
				...
				info->finger_cnt--;
				if(!info->finger_cnt)
					queue_work(info->tb_work_queue, &info->tb_off);
#endif
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
...
#if TOUCH_BOOSTER
				if(!info->finger_cnt)
					queue_work(info->tb_work_queue, &info->tb_on);
				info->finger_cnt++;
#endif
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
...
#endif
#if TOUCH_BOOSTER
	flush_workqueue(info->tb_work_queue);
...
#ifdef TOUCH_BOOSTER
static void touch_booster_on(struct work_struct *work)
{
	mutex_lock(&touch_booster_lock);
	if (!misc_info->min_handle) {
		misc_info->min_handle = cpufreq_limit_min_freq(touch_cpufreq_lock, "TSP");
		if (IS_ERR(misc_info->min_handle)) {
			dev_err(&misc_info->client->dev,"[TSP] cannot get cpufreq_min lock %lu(%d)\n",
					touch_cpufreq_lock, PTR_ERR(misc_info->min_handle));
			misc_info->min_handle = NULL;
		}
		_store_cpu_num_min_limit(2);

		dev_info(&misc_info->client->dev,"cpu freq on\n");
	}
	mutex_unlock(&touch_booster_lock);
}
static void touch_booster_off(struct work_struct *work)
{
	mutex_lock(&touch_booster_lock);
	if (misc_info->min_handle) {
		cpufreq_limit_put(misc_info->min_handle);
		misc_info->min_handle = NULL;
		_store_cpu_num_min_limit(1);
		dev_info(&misc_info->client->dev,"cpu freq off\n");
	}
	mutex_unlock(&touch_booster_lock);
}
#endif
...
static void get_chip_type(void *device_data)
{
	...
	case 0:
		snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s", "ITO");
		break;
	case 3:
		snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s", "MI");
		break;
	default: break;
	}
	set_cmd_result(info, finfo->cmd_buff,
...
static void run_reference_PDiff(void *device_data)
{
	...
	// if button's position is a minus, didn't check it.
	// ex) degas have to set like this,  run_reference_PDiff_read(device_data, hArr, bArr, -1, 4, 13, -1);
...
/*	input_set_abs_params(info->input_dev, ABS_MT_WIDTH_MINOR,
	...
	info->work_state = NOTHING;
#if TOUCH_BOOSTER
	INIT_WORK(&info->tb_on, touch_booster_on);
	INIT_WORK(&info->tb_off, touch_booster_off);
	info->tb_work_queue = create_singlethread_workqueue("touch_booster");
	if (info->tb_work_queue == NULL) {
		printk("create_singlethread_workqueue for touch_booster failed!\n");
		goto failed_to_create_tb_work_queue;
	}
	info->finger_cnt = 0;
#endif
...
failed_to_create_tb_work_queue:
