...
{
	SENSOR_PRINT_HIGH("SENSOR : _sensor_k_kzalloc : E");
...
LOCAL void _sensor_k_kfree(void *p)
{
	...
	SENSOR_PRINT_HIGH("SENSOR : _sensor_k_kfree : E");
	return;
}
...
LOCAL int sensor_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	SENSOR_PRINT_HIGH(KERN_INFO "SENSOR : sensor_probe, addr 0x%x\n", s_p_sensor_mod->cur_i2c_client->addr);
	return 0;
out:
	return res;
}
...
{
	SENSOR_PRINT_HIGH("SENSOR : sensor_remove : E");
	return 0;
}

LOCAL int sensor_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	SENSOR_PRINT_HIGH("SENSOR : sensor_detect : E");
...
int sensor_k_set_voltage_cammot(uint32_t cammot_val)
{
	...
	if (NULL == s_p_sensor_mod->cammot_regulator) {
		...
		{
			SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_cammot : Get cammot fail\n");
	...
	case SENSOR_VDD_2800MV:
		...
		if (err)
				SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_cammot : Set cammot 2.8 fail\n");
		break;
	case SENSOR_VDD_3000MV:
		...
		if (err)
				SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_cammot : Set cammot 3.0 fail\n");
		break;
#if defined (CONFIG_ARCH_SCX35)
	case SENSOR_VDD_3300MV:
		...
		if (err)
				SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_cammot : Set cammot 3.3 fail\n");
		break;
#else
	case SENSOR_VDD_2500MV:
		...
		if (err)
				SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_cammot : Set cammot 2.5 fail\n");
		break;
#endif
	case SENSOR_VDD_1800MV:
		...
		if (err)
				SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_cammot : Set cammot 1.8 fail\n");
		break;
	...
	if (err)
	{
		SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_cammot : Set cammot error\n");
		return SENSOR_K_FAIL;
	}
	if (0 != volt_value) {
		...
		if (err) {
			...
			SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_cammot : Can't enable cammot\n");
			return SENSOR_K_FAIL;
		}
	} else {
		...
		regulator_put(s_p_sensor_mod->cammot_regulator);
		s_p_sensor_mod->cammot_regulator = NULL;
		SENSOR_PRINT("SENSOR : sensor_k_set_voltage_cammot : Disable cammot\n");
	}
...
int sensor_k_set_voltage_avdd(uint32_t avdd_val)
{
	...
	if (NULL == s_p_sensor_mod->camavdd_regulator) {
		...
		{
			SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_avdd : Get avdd fail\n");
			return SENSOR_K_FAIL;
		}
	}
	switch (avdd_val) {
	case SENSOR_VDD_2800MV:
		...
		volt_value = SENSOER_VDD_2800MV;
		if (err)
				SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_avdd : Set avdd to 2.8 fail\n");
		break;
	case SENSOR_VDD_3000MV:
		err = regulator_set_voltage(s_p_sensor_mod->camavdd_regulator,
					SENSOER_VDD_3000MV,
		...
		volt_value = SENSOER_VDD_3000MV;
		if (err)
				SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_avdd : Set avdd to 3.0 fail\n");
		break;
	case SENSOR_VDD_2500MV:
		err = regulator_set_voltage(s_p_sensor_mod->camavdd_regulator,
					SENSOER_VDD_2500MV,
		...
		volt_value = SENSOER_VDD_2500MV;
		if (err)
				SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_avdd : Set avdd to 2.5 fail\n");
		break;
	case SENSOR_VDD_1800MV:
		err = regulator_set_voltage(s_p_sensor_mod->camavdd_regulator,
					SENSOER_VDD_1800MV,
		...
		volt_value = SENSOER_VDD_1800MV;
		if (err)
				SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_avdd : Set avdd to 1.8 fail\n");
		break;
	case SENSOR_VDD_CLOSED:
	case SENSOR_VDD_UNUSED:
	default:
	...
	if (err)
	{
		SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_avdd : Set avdd error\n");
		return SENSOR_K_FAIL;
	}
	if (0 != volt_value) {
		...
		if (err) {
			...
			SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_avdd : Can't enable avdd\n");
			return SENSOR_K_FAIL;
		}
	} else {
		...
		regulator_put(s_p_sensor_mod->camavdd_regulator);
		s_p_sensor_mod->camavdd_regulator = NULL;
		SENSOR_PRINT("SENSOR : sensor_k_set_voltage_avdd : Disable avdd\n");
	}
	return SENSOR_K_SUCCESS;
}
...
#if  defined (CONFIG_MACH_CORE3_W) && !defined( CONFIG_SENSOR_OUTPUT_RAW)
...
if (SENSOR_MAIN == _sensor_K_get_curId()) {
	...
	if (!s_p_sensor_mod->camdvdd_regulator) {
		...
		{
			SENSOR_PRINT_ERR("SENSOR : _sensor_k_setvoltage_dvdd : Get DVDD fail\n");
			return SENSOR_K_FAIL;
		}
	}
	switch (dvdd_val) {
	...
	if (0 != volt_value) {
		...
		if (err) {
			...
			SENSOR_PRINT_ERR("SENSOR : Can't enable DVDD\n");
			return SENSOR_K_FAIL;
		}
	} else {
		...
		regulator_put(s_p_sensor_mod->camdvdd_regulator);
		s_p_sensor_mod->camdvdd_regulator = NULL;
		SENSOR_PRINT("SENSOR : Disable DVDD\n");
	}
	return SENSOR_K_SUCCESS;
}
...
int sensor_k_set_voltage_iovdd(uint32_t iodd_val)
{
	...
	if(NULL == s_p_sensor_mod->camvio_regulator) {
		...
		{
			SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_iovdd : Get camvio fail\n");
			return SENSOR_K_FAIL;
		}
	}
	switch (iodd_val) {
	case SENSOR_VDD_2800MV:
		...
		volt_value = SENSOER_VDD_2800MV;
		if (err)
				SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_iovdd : Set camvio to 2.8 fail\n");
		break;
	...
	case SENSOR_VDD_2500MV:
		...
		volt_value = SENSOER_VDD_2500MV;
		if (err)
				SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_iovdd : Set camvio to 2.5 fail\n");
		break;
	case SENSOR_VDD_1500MV:
		err = regulator_set_voltage(s_p_sensor_mod->camvio_regulator,
					SENSOER_VDD_1500MV,
		...
		volt_value = SENSOER_VDD_1500MV;
		if (err)
				SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_iovdd : Set camvio to 1.5 fail\n");
		break;
#else
	case SENSOR_VDD_3800MV:
		...
		if (err)
				SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_iovdd : Set camvio to 3.8 fail\n");
		break;
	...
	case SENSOR_VDD_1200MV:
		...
		volt_value = SENSOER_VDD_1200MV;
		if (err)
				SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_iovdd : Set camvio to 1.2 fail\n");
		break;
	...
	case SENSOR_VDD_1800MV:
		...
		volt_value = SENSOER_VDD_1800MV;
		if (err)
				SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_iovdd : Set camvio to 1.8 fail\n");
		break;
	case SENSOR_VDD_CLOSED:
	case SENSOR_VDD_UNUSED:
	default:
	...
	if (err)
	{
		SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_iovdd : Set camvio err\n");
		return SENSOR_K_FAIL;
	}
	if (0 != volt_value) {
		...
		if (err) {
			...
			SENSOR_PRINT_ERR("SENSOR : sensor_k_set_voltage_iovdd : Can't enable camvio\n");
			return SENSOR_K_FAIL;
		}
	} else {
		...
		regulator_put(s_p_sensor_mod->camvio_regulator);
		s_p_sensor_mod->camvio_regulator = NULL;
		SENSOR_PRINT("SENSOR : sensor_k_set_voltage_iovdd : Disable camvio\n");
	}
...
LOCAL int _select_sensor_mclk(uint8_t clk_set, char **clk_src_name, uint8_t * clk_div)
{
	...
	SENSOR_PRINT("SENSOR : _select_sensor_mclk : Sel mclk %d\n", clk_set);
...
int32_t _sensor_k_mipi_clk_en(struct device_node *dn)
{
	...
	{
		printk("SENSOR : _sensor_k_mipi_clk_en : Get dcam mipi clk error \n");
		return -1;
	} else {
...
int sensor_k_set_mclk(uint32_t mclk)
{
	...
	SENSOR_PRINT_HIGH("SENSOR : sensor_k_set_mclk : Set mclk org = %d, clk = %d\n", s_p_sensor_mod->sensor_mclk, mclk);
	...
	if ((0 != mclk) && (s_p_sensor_mod->sensor_mclk != mclk)) {
		...
		{
			SENSOR_PRINT_ERR("SENSOR : sensor_k_set_mclk : Sensor_SetMCLK select clock source fail\n");
			return -EINVAL;
		}
		...
		s_p_sensor_mod->sensor_mclk = mclk;
		SENSOR_PRINT("SENSOR : sensor_k_set_mclk : Set mclk %d Hz\n", s_p_sensor_mod->sensor_mclk);
	}
	else if (0 == mclk)
	{
		if (s_p_sensor_mod->sensor_mclk)
		...
		SENSOR_PRINT("SENSOR : sensor_k_set_mclk : Disable MCLK !!!");
	}
	else
	{
		SENSOR_PRINT("SENSOR : sensor_k_set_mclk : Do nothing !!");
	}
	SENSOR_PRINT_HIGH("SENSOR : sensor_k_set_mclk : X\n");
...
LOCAL int _sensor_k_reset(uint32_t level, uint32_t width)
{
	...
	case SENSOR_MAIN:
	{
			SENSOR_PRINT_HIGH("SENSOR : _sensor_k_reset : MAIN RST reset_val = %d\n", level);
	...
	case SENSOR_SUB:
	{
			SENSOR_PRINT_HIGH("SENSOR : _sensor_k_reset : Sub RST reset_val = %d\n", level);
...
int sensor_k_set_rst_level(uint32_t plus_level)
{
	switch (_sensor_K_get_curId())
	{
		case SENSOR_MAIN:
		{
			SENSOR_PRINT_HIGH("SENSOR : sensor_k_set_rst_level : Main RST level is %d, rst pin = %d \n", plus_level, s_p_sensor_mod->pin_main_reset);
		...
		case SENSOR_SUB:
		{
			SENSOR_PRINT_HIGH("SENSOR : sensor_k_set_rst_level : Sub RST level is %d, rst pin = %d \n", plus_level, s_p_sensor_mod->pin_sub_reset);
...
LOCAL int _Sensor_K_ReadReg(SENSOR_REG_BITS_T_PTR pReg)
{
	...
	for (i = 0; i < SENSOR_I2C_OP_TRY_NUM; i++) {
		...
		if (ret != 2)
		{
			SENSOR_PRINT_ERR("SENSOR : _Sensor_K_ReadReg : Read reg fail, ret %d, addr 0x%x, reg_addr 0x%x\n", ret, s_p_sensor_mod->cur_i2c_client->addr, reg_addr);
			SLEEP_MS(20);
			ret = SENSOR_K_FAIL;
		}
		else
...
LOCAL int _sensor_k_get_flash_level(SENSOR_FLASH_LEVEL_T *level)
{
	...
	SENSOR_PRINT("Sensor : _sensor_k_get_flash_level : Sensor get flash level : low %d, high %d\n", level->low_light, level->high_light);
	return SENSOR_K_SUCCESS;
}
...
LOCAL int _sensor_k_wr_regtab(SENSOR_REG_TAB_PTR pRegTab)
{
	...
	if (PNULL == pBuff) {
		ret = SENSOR_K_FAIL;
		SENSOR_PRINT_ERR("Sensor : _sensor_k_wr_regtab : Alloc fail, cnt %d, size %d\n", cnt, size);
		goto _Sensor_K_WriteRegTab_return;
	}
	else
	{
		SENSOR_PRINT("Sensor : _sensor_k_wr_regtab : Alloc success, cnt %d, size %d \n",cnt, size);
	}

	if (copy_from_user(pBuff, pRegTab->sensor_reg_tab_ptr, size)) {
		ret = SENSOR_K_FAIL;
		SENSOR_PRINT_ERR("Sensor : _sensor_k_wr_regtab : Copy user fail, size %d\n", size);
		goto _Sensor_K_WriteRegTab_return;
	}
...
LOCAL int _sensor_k_set_i2c_clk(uint32_t clock)
{
	...
	SENSOR_PRINT("Sensor : _sensor_k_set_i2c_clk : Sensor set i2c clk %d\n", clock);
	return SENSOR_K_SUCCESS;
}
...
LOCAL int _sensor_k_wr_i2c(SENSOR_I2C_T_PTR pI2cTab)
{
	...
	if (ret != 1)
	{
		SENSOR_PRINT_ERR("SENSOR : _sensor_k_wr_i2c : Write reg fail, ret : %d, addr : 0x%x\n", ret, msg_w.addr);
	}
	else
	{
		ret = SENSOR_K_SUCCESS;
...
int _sensor_k_rd_i2c(SENSOR_I2C_T_PTR pI2cTab)
{
	...
	for (i = 0; i < SENSOR_I2C_OP_TRY_NUM; i++) {
		...
		if (ret != 2)
		{
			SENSOR_PRINT_ERR("SENSOR : _sensor_k_rd_i2c : Read reg fail, ret %d, addr 0x%x\n", ret, s_p_sensor_mod->cur_i2c_client->addr);
			SLEEP_MS(20);
			ret = SENSOR_K_FAIL;
		} else {
			ret = SENSOR_K_SUCCESS;
			if (copy_to_user(pI2cTab->i2c_data, pBuff, read_num)) {
				ret = SENSOR_K_FAIL;
				SENSOR_PRINT_ERR("SENSOR : _sensor_k_rd_i2c : sensor W I2C ERR: copy user fail, size %d \n", cnt);
				goto sensor_k_readi2c_return;
			}
			break;
		}
...
LOCAL ssize_t sensor_k_write(struct file *filp, const char __user *ubuf, size_t cnt, loff_t *gpos)
{
	...
	SENSOR_PRINT("Sensor : sensor_k_write : Write count %d, buf %d\n", cnt, sizeof(buf));
	...
	if (cnt < sizeof(buf)) {
		...
		if (PNULL == pBuff)
		{
			SENSOR_PRINT_ERR("Sensor : sensor_k_write : Write error : Alloc fail, size %d \n", cnt);
			goto sensor_k_write_return;
		}
		else
		{
			SENSOR_PRINT("Sensor : sensor_k_write : Alloc success, size %d \n", cnt);
		}
	}

	if (copy_from_user(pBuff, ubuf, cnt))
	{
		SENSOR_PRINT_ERR("Sensor : sensor_k_write : Write error : Copy user fail, size %d\n", cnt);
		goto sensor_k_write_return;
	}

	printk("Sensor : sensor_k_write : Client addr 0x%x\n", s_p_sensor_mod->cur_i2c_client->addr);
	msg_w.addr = s_p_sensor_mod->cur_i2c_client->addr;
	msg_w.flags = 0;
	msg_w.buf = pBuff;
	msg_w.len = cnt;
	...
	if (ret != 1)
	{
		SENSOR_PRINT_ERR("SENSOR : sensor_k_write : Write reg fail, ret %d, Write addr : 0x%x\n", ret, s_p_sensor_mod->cur_i2c_client->addr);
	}
	else
	{
		ret = SENSOR_K_SUCCESS;
...
sensor_k_write_return:
	...
	SENSOR_PRINT("Sensor : sensor_k_write : Write done, ret %d\n", ret);
...
int _sensor_burst_write_init(SENSOR_REG_T_PTR p_reg_table, uint32_t init_table_size)
{
	...
	if (0 == i2c_client)
	{
		SENSOR_PRINT_ERR("SENSOR : _sensor_burst_write_init : Burst write Init err, i2c_clnt NULL\n");
		return -1;
	}
...
int _sensor_burst_write_init(SENSOR_REG_T_PTR p_reg_table, uint32_t init_table_size)
{
	...
	printk("SENSOR : _sensor_burst_write_init : E\n");
	if (0 == i2c_client)
	{
		SENSOR_PRINT_ERR("SENSOR : _sensor_burst_write_init : Burst write Init error, I2C client is NULL\n");
		return -1;
	}
	...
	{
		SENSOR_PRINT_ERR("SENSOR : _sensor_burst_write_init ERROR : Alloc is fail, size = %d\n", init_table_size * sizeof(uint16_t) + 16);
		return -1;
	}
	else
	{
		SENSOR_PRINT_HIGH("SENSOR : _sensor_burst_write_init : Alloc success, size = %d \n", init_table_size * sizeof(uint16_t) + 16);
	}
	...
	while (written_num < init_table_size) {
		...
		if (SENSOR_WRITE_DELAY == wr_reg) {
			...
			if (ret!=1)
			{
				SENSOR_PRINT("SENSOR : _sensor_burst_write_init : s err, val {0x%x 0x%x} {0x%x 0x%x} {0x%x 0x%x} {0x%x 0x%x} {0x%x 0x%x} {0x%x 0x%x}\n",
					...
					SENSOR_PRINT("SENSOR : _sensor_burst_write_init : I2C write once err\n");
				rtn = 1;
				break;
			}
		}
	...
	SENSOR_PRINT("SENSOR : _sensor_burst_write_init : Burst write Init OK\n");
...
{
	printk(KERN_INFO "Sensor : sensor_k_remove : Sensor remove called !\n");
...
#if defined(CONFIG_MACH_CORE3_W) || defined(CONFIG_MACH_YOUNG23GDTV) || defined(CONFIG_MACH_GRANDNEOVE3G) || defined(CONFIG_MACH_VIVALTO5MVE3G) || defined(CONFIG_MACH_J13G)
...
static DEVICE_ATTR(rear_camfw, 0664, Rear_Cam_Sensor_ID, NULL);
static DEVICE_ATTR(rear_type, 0664, Cam_Sensor_TYPE, NULL);
#if !defined(CONFIG_MACH_YOUNG23GDTV) && !defined(CONFIG_MACH_J13G)
static DEVICE_ATTR(rear_flash, 0664, Rear_Cam_show_flash, Rear_Cam_store_flash);
static DEVICE_ATTR(front_camfw, 0664, Front_Cam_Sensor_ID, NULL);
static DEVICE_ATTR(front_type, 0664, Cam_Sensor_TYPE, NULL);
...
{
	printk(KERN_INFO "Sensor : sensor_k_init : E\n");
...
{
	printk(KERN_INFO "sensor_k_exit : E\n");
	platform_driver_unregister(&sensor_dev_driver);

	if (SENSOR_ADDR_INVALID(s_p_sensor_mod))
	{
		printk("SENSOR : sensor_k_exit : Invalid addr, 0x%x", (uint32_t)s_p_sensor_mod);
	}
	else
	{
		vfree(s_p_sensor_mod);
