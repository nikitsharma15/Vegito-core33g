...
#if IST30XX_NODE_CAP
#define IST30XX_CR_JITTER_RESULT_ADDR	(0x2000046C)

static int ist30xx_check_node_cap(char* buf, u32* rawbuf, size_t size)
{
	int	ret = -1;
	int	i, j;
	char	msg[128];
	...
	ist30xx_reset(false);
	ist30xx_write_cmd(ts_data->client, CMD_MODE_FOR_JIG, 0x41);

	msleep(6000);
	ret = ist30xxb_burst_read(ts_data->client, IST30XXB_DA_ADDR(IST30XXB_RAW_ADDR + 0xD4), rawbuf, size);
	if (ret != 0)
		return ret;
...
static int ist30xx_run_node_cap(char* buf, u32* rawbuf, size_t size)
{
	int	ret = -1;
	...
	for (i = 0; i < tsp->ch_num.tx; i++) {
		for (j = 0; j < tsp->ch_num.rx; j++) {
			...
			cnt += sprintf(msg, "%4d", *(rawbuf + (tsp->ch_num.rx * i + j)));
			strcat(buf, msg);	
		}
		strcat(buf, "\n");
		cnt++;
...
/* sysfs: /sys/class/touch/node_cap */
static ssize_t ist30xx_node_cap_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	int ret;
	...
	mutex_lock(&ist30xx_mutex);
	ret = ist30xx_check_node_cap(buf, ist30xx_frame_rawbuf, NODE_TOTAL_NUM);
	mutex_unlock(&ist30xx_mutex);
...
#if IST30XX_NODE_CAP
static DEVICE_ATTR(node_cap, S_IRUGO, ist30xx_node_cap_show, NULL);
#endif
...
#if IST30XX_NODE_CAP
struct device *ist30xx_node_cap_dev;
#endif
