...
static void ESD_work_func(struct work_struct *work)
{
	...
	if(0 != dev->enable){
		...
		queue_delayed_work(system_power_efficient_wq,
                        &dev->ESD_work, msecs_to_jiffies(dev->ESD_timeout_val));
...
static ssize_t show_lcd_info(struct device *dev, struct device_attribute *attr, char *buf)
{
	int DBh_value = (lcd_id_from_uboot >> 8) & 0xFF;

	if(DBh_value & 0x04)
	{
		return sprintf(buf, "INH_%X%X%X",(lcd_id_from_uboot >> 16) & 0xFF,(lcd_id_from_uboot >> 8) & 0xFF, lcd_id_from_uboot  & 0xFF);
	}
	else
	{
		switch(DBh_value >> 3)
		{
		case 0x08 : //CPT
			return sprintf(buf, "CPT_%X%X%X",(lcd_id_from_uboot >> 16) & 0xFF,(lcd_id_from_uboot >> 8) & 0xFF, lcd_id_from_uboot  & 0xFF);
			break;
		case 0x09 : //AUO
			return sprintf(buf, "AUO_%X%X%X",(lcd_id_from_uboot >> 16) & 0xFF,(lcd_id_from_uboot >> 8) & 0xFF, lcd_id_from_uboot  & 0xFF);
			break;
		case 0x14 : //TIANMA
			return sprintf(buf, "TIA_%X%X%X",(lcd_id_from_uboot >> 16) & 0xFF,(lcd_id_from_uboot >> 8) & 0xFF, lcd_id_from_uboot  & 0xFF);
			break;
		case 0x17 : //BOE
			return sprintf(buf, "BOE_%X%X%X",(lcd_id_from_uboot >> 16) & 0xFF,(lcd_id_from_uboot >> 8) & 0xFF, lcd_id_from_uboot  & 0xFF);
			break;
		case 0x18 : //DTC
			return sprintf(buf, "DTC_%X%X%X",(lcd_id_from_uboot >> 16) & 0xFF,(lcd_id_from_uboot >> 8) & 0xFF, lcd_id_from_uboot  & 0xFF);
			break;
		default :
			return sprintf(buf, "Unknow. need to check %X", lcd_id_from_uboot);
			break;
		}
	}
}
...
static int sprdfb_probe(struct platform_device *pdev)
{
	...
	if (!lcd_is_dummy) {
		...
		queue_delayed_work(system_power_efficient_wq,
                        &enable_esd_work, msecs_to_jiffies(20000));
	}
#endif
	return 0;
