...
cycle_t __gptimer_clocksource_read(struct clocksource *cs)
{
	...
	cycle_t cycle_last = cs->cycle_last;

	spin_lock_irqsave(&clocksource_read_lock, flags);
	unsigned long v = ~readl_relaxed(TIMER_CNT_RD(e_cpu, SOURCE_TIMER));

	/* W/A for abnormal cnt case*/
	dt = (long)(v-last_v);
	while(dt < 0 || (int)(v - cycle_last) < 0){
		printk("%s: v = 0x%0.8x, last_v = 0x%0.8x, dt = %d, cycle_last = %lld, v - cycle_last = %d, discard v\n",
			__func__, v, last_v, dt, cycle_last, (int)(v - cycle_last));
		v = ~readl_relaxed(TIMER_CNT_RD(e_cpu, SOURCE_TIMER));
		dt = (long)(v-last_v);
		printk("%s: v = 0x%0.8x, last_v = 0x%0.8x, dt = %d, cycle_last = %lld, v - cycle_last = %d\n",
			 __func__, v, last_v, dt, cycle_last, (int)(v - cycle_last));
	}
	last_v = v;
