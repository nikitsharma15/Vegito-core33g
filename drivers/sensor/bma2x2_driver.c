		...
		struct bma2x2_data *client_data)
{
	...
	bsd.z = val->z;
#if defined (CONFIG_MACH_GRANDNEOVE3G)
	bst_remap_sensor_data_dft_tab(&bsd,client_data->place);
#else
...
#if defined (CONFIG_MACH_GRANDNEOVE3G)
	bma_vio_reg = data->reg_name;
#else
	bma_vio_reg = "bma2x2,vdd";
#endif
...
#endif
	data->reg_vio = devm_regulator_get(&data->bma2x2_client->dev, bma_vio_reg);
...
static int bma2x2_parse_dt(struct bma2x2_data *data, struct device *dev) {
	...
	ret = of_property_read_string(np, "bma2x2,bma2x2-vdd",&data->reg_name);
	if (ret < 0) {
		pr_err("%s - get regulator name error\n", __func__);
		return -ENODEV;
	}
